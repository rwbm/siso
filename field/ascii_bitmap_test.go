...
func TestAsciiBitmapEncodePrimary(t *testing.T) {
	...
	encoded, err := NewBitmap(bitmap).Encode()
	if err != nil {
...
func TestAsciiBitmapEncodeRejectsSecondaryWithoutBits(t *testing.T) {
	...
	if _, err := NewBitmap(bitmap).Encode(); err == nil {
...
func TestAsciiBitmapDecodePrimary(t *testing.T) {
	...
	field := NewBitmap("")
	if err := field.Decode([]byte(hexBitmap)); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if field.Value() != bitmap {
		t.Fatalf("decoded bitmap mismatch: got %q want %q", field.Value(), bitmap)
	}
}
...
func TestAsciiBitmapDecodeSecondary(t *testing.T) {
	...
	if len(field.Value()) != 128 {
		t.Fatalf("expected 128 bits, got %d", len(field.Value()))
	}
...
func TestAsciiBitmapDecodeMissingSecondary(t *testing.T) {
	...
	if err := NewBitmap("").Decode([]byte("8000000000000000")); err == nil {
...
func TestAsciiBitmapDecodeRejectsInvalidHex(t *testing.T) {
	if err := NewBitmap("").Decode([]byte("ZZ00000000000000")); err == nil {
